...
function OrderDetails(props: TOrderDetailstypeprops) {
  ...
  const newBurgerOrder: TNewBurgerConstructor =
    useAppSelector(getNewBurgerSelector);
  ...
  return (
    <section className={styles.orderDetails}>
      ...
      {newOrder === "0" && (
        <div className={`${styles.text} text text_type_main-small`}>
          {newOrderBar}
        </div>
      )}
      <p className=" mt-8 mb-15 text text_type_main-medium">{props.one}</p>
      {newOrder !== "0" && <img src={icon} alt="Галка" />}
